...
hist(data$X.1c.,xlab="Travel Kilometers",main="", breaks=50, col="gray")
hist(data$X.1c.2,add=T, col="#f9af23", breaks=50)
legend("topright", c("Not Interested", "Interested"), fill=c("gray", "#f9af23"))
...
hist(data$X5b,"mins",xlab="Arrival and Departure Time",col=rgb(.5,.5,.5,0.4), breaks=50, xlim=as.POSIXct(c(startHour,endHour)),xaxt='n', ann=FALSE)
hist(data$X5a,"mins",xlab="Arrival Time",add=T, col=rgb(.98,.69,.14,0.6), breaks=35, xaxt='n', ann=FALSE)
...
plot(col=rgb(.98,.69,.14,1), data$X.1c.,data$X6a, xlab="Travel Distance", ylab="Willingsness to Pay for total Distance",xlim = c(0,90))
...
legend("topright", c("Average Traveling Costs by Car (0,6\u20ac/km)","German Tax Refunds (0,3\u20ac/km)","Fitted Willingsness to Pay"), fill=c("green","blue",rgb(.98,.69,.14,1)),cex = 1.2)
...
plot(col="darkgray", data$X.1c.,data$X7a, xlab="Travel Distance", ylab="Expected Payout in Euro for total Distance",xlim = c(0,120),ylim = c(0,10))
eppklm = lm(X7a ~ X.1c., data=data)
summary(eppklm)$sigma #1.066353
abline(eppklm,col="darkgray")
abline(0,0.3,col="blue")
abline(0,0.6,col="green")
legend("topright", c("Average Traveling Costs by Car (0,6\u20ac/km)","German Tax Refunds (0,3\u20ac/km)","Fitted Expected Payout"), fill=c("green","blue", "darkgray"),cex = 1.2)
